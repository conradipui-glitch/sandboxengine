...
## 24. Состояние выполнения

Этот файл задаёт требования и не является журналом текущей реализации. Актуальные принятые блоки, SHA, CI и ограничения находятся в [`docs/STATUS.md`](STATUS.md), а точка продолжения — в [`docs/HANDOFF.md`](HANDOFF.md). История B12 сохраняется в release-документах; состояние B13 устанавливается только отдельной приёмкой по §25 и T33–35.
