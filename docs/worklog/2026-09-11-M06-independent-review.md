...
## Браузерный проход по живому сайту (2026-09-11)

Реальный Chromium (headless, CDP через `scripts/l07-cdp.mjs`, viewport 1280×900) против задеплоенного Worker:

| Шаг | Наблюдение |
|---|---|
| Каталог | 4 кнопки «Играть — …»: `M06 Mission Alpha` **плюс** три legacy-истории — аддитивность каталога подтверждена глазами |
| Старт опубликованной миссии | карточка `1855 · Севастополь — M06 Mission Alpha`, переход в игру, `К списку миссий`, шапка «M06 Mission Alpha / корреспондент · Ход 0» |
| Ход 0 | кадр `scene`: «Палатка лагеря», текст «Ты у входа в лагерь.», выбор `.mp-choice` «Осмотреть лагерь» |
| Ход 1 | «Бастион», текст «Ты у бастиона.», выбор «Завершить» |
| Ход 2 | финал: «Отчёт написан» / «История завершена.» |
| Reload после финала | финал сохранён («Отчёт написан … История завершена.») — F04 подтверждён в браузере |
| Консоль | 0 ошибок и 0 исключений на всём проходе |

Замечание по внешнему виду (не дефект рендера): опубликованная тестовая миссия визуально аскетична — `<div class="mp-background mp-background-empty">`, `.mp-grid` и подпись. Причина в самом авторском контенте: BFF отдаёт `backgroundUrl: null`, `layers: []`, `musicTitle: null`. Renderer отображает ровно то, что есть в ревизии; чтобы проверить «богатый» вид, нужна миссия с ассетами (публичный маршрут ассетов готов, тест `m06-public-asset.test.mjs` 1/1).
