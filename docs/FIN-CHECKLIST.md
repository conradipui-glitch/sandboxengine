...
| FIN-03 старые сессии сохраняют ассеты | B04, M03/M06 | PARTIAL | engine-половина: session-pinned маршрут `/public/v1/missions/:id/sessions/:sid/assets/:assetId` отдаёт байты ревизии, к которой привязана сессия (до фикса запрос → 404); credential сессии обязателен (нет/подделка → 401, чужая сессия/миссия → 404), отдача `private`; тест `apps/server/test/fin03-session-assets.test.mjs` 1/1 (RED 0/1 с отключённым маршрутом); Server 142/142. site-половина — ветка `feat/fin03-site-assets` (`fc87d45`, отдельный репозиторий) | стыковка site↔engine на session-pinned URL; hosted-прогон через nginx под gate |
...
| FIN-05 экраны и ручная композиция | M03–M05, F05 | PARTIAL | доска сюжета: сцены/выборы create·rename·duplicate·delete, подписи стрелок (решение→переход), undo/redo + клавиатура, «Вписать всё» и zoom к курсору, layout≠contentHash — `apps/studio/test/fin05-story-board.test.mjs` 15/15; `test:studio` 126/0/1 | слои/transform/анимация/аудио (drag/resize/rotate/flip/opacity/z/override, preset, reduced-motion, музыка), 3 сцены + 2 финала не-Florence, реальная библиотека материалов, browser-приёмка |
...
- FIN-05: браузерный проход gated Studio с реальным входом не выполнялся (gate не ослаблялся, credentials не подбирались); есть regression-тест dev:studio с настоящим Control и DOM-шим-тест `story-dom.ts`. Композиция слоёв/анимация/аудио и приёмка 3+ сцен/2 финалов не закрыты (см. `docs/worklog/2026-09-11-FIN05-studio-board.md`).
