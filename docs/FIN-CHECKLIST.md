...
| FIN-05 экраны и ручная композиция | M03–M05, F05 | PARTIAL | доска сюжета: сцены/выборы create·rename·duplicate·delete, подписи стрелок (решение→переход), undo/redo + клавиатура, «Вписать всё» и zoom к курсору, layout≠contentHash — `apps/studio/test/fin05-story-board.test.mjs` 15/15. Композиция экрана (FIN-05B): слои drag/resize(пропорц.)/rotate/flip/opacity/z-order/duplicate/delete/lock/visible, contain/cover + фокус и crop, наследование фона (own/inherited/none), preset + reduced-motion/пауза, реальные mute/play/blocked для музыки, клавиатура (стрелки/Shift, [ ] { }, V, L, D, Del, Esc), запись через тот же CAS `/mission` — `apps/studio/test/fin05b-screen-composition.test.mjs` 16/16; `test:studio` 142/0/1; Control 106/106, Server 141/141, Contracts 57/57 | реальная библиотека материалов (hash/MIME/размеры, restart/другой браузер), экраны intro/scene/dialogue/choice/ending в реальном runtime/BFF, browser-приёмка (3+ сцены, 2 ветви, 2 финала не-Florence) |
