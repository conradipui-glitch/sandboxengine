...
Активные карточки: **FIN-06 (M06, C18) — права LOCAL_PASS, живые роли PARTIAL** (`191ba30`: `docs/CAPABILITY-MATRIX.md` + `apps/server/test/fin06-role-matrix-http.test.mjs` 3/3), **FIN-02 (B03) — LOCAL_PASS** (`9caec73`), **FIN-03 (B04) — engine-половина LOCAL_PASS, site-половина в ветке сайта**, **FIN-04 (B01) — PARTIAL**, **FIN-05 + FIN-05B + FIN-12 (сервер и UI) влиты** (`b20d72b`).
Первый незакрытый критерий: живой браузерный проход авторского маршрута в Studio (FIN-05: реальная библиотека материалов, экраны в runtime/BFF, приёмка 3+ сцены/2 ветви/2 финала) — gated Studio не вскрывался, credentials не подбирались; затем hosted-прогон FIN-01/FIN-02/FIN-03 через nginx (нужна доставка новой ревизии engine — FIN-04); живые роли C18 — по приглашённым аккаунтам; FIN-07/08 — нужны реальные ключи провайдеров.
Следующее действие: пройти авторский маршрут в живом браузере на локально поднятом Studio с настоящим Control (создание миссии → доска → экраны → preview → проверка), записать экраны и по найденным препятствиям закрыть остаток FIN-05; hosted-доставка — только по отдельному разрешению владельца.
Текущий SHA рабочей ветки: `191ba30` (FIN-06 rights); до него `1b1b922` (docs FIN-02), `9caec73` (FIN-02 recovery), `073ef18` (docs FIN-12/FIN-05), `b20d72b` (FIN-12 UI merge), `440e7d6` (FIN-04 repro), `01cbbd6` (FIN-04 манифест), `523dca5` (FIN-05), `232afa2` (FIN-02), `1886041` (FIN-01).
