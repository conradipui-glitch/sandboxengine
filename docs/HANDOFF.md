...
Активные карточки: **FIN-02 (B03) — LOCAL_PASS, восстановление после падения между стадиями закрыто** (`9caec73`), **FIN-03 (B04, ассеты открытых сессий) — engine-половина LOCAL_PASS, site-половина в ветке сайта**, **FIN-04 (B01, доставка) — манифест состава сделан, карточка PARTIAL**, **FIN-05 + FIN-05B + FIN-12 (сервер и UI) влиты** (`b20d72b`).
Первый незакрытый критерий: hosted-прогон FIN-01/FIN-02/FIN-03 через nginx под gate (нужна доставка новой ревизии engine — FIN-04), затем FIN-06 (capability-матрица + backend negative tests; живые роли editor/viewer/read-only — по приглашённым аккаунтам), FIN-07/08 (ИИ-провайдеры, нужны реальные ключи).
Следующее действие: доставка `feat/b13-acceptance-closure` на VPS по exact-SHA + перегенерация `deploy/vps/delivery-manifest.json`, затем hosted smoke (включая `docker compose restart` с прерванной публикацией) и сквозная приёмка.
Текущий SHA рабочей ветки: `9caec73` (FIN-02 recovery); до него `073ef18` (docs FIN-12/FIN-05), `b20d72b` (FIN-12 UI merge), `440e7d6` (FIN-04 repro), `01cbbd6` (FIN-04 манифест), `523dca5` (FIN-05), `232afa2` (FIN-02), `1886041` (FIN-01).
