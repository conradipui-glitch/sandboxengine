...
Текущая работа в `feat/live-author-studio`: [L00–L09 — завершение live authoring](tasks/LIVE-AUTHOR-COMPLETION.md). L00–L02 приняты; выполняется L03. Сквозной тест через HTTP adapter, браузер, live API и кнопка Player ещё не приняты. Это не меняет исторические статусы релиза ниже. B13 — последующий, не текущий шаг.
...
| Studio/Player | **GREEN / live-authoring in progress** | historical author/playtest/publication paths + T29 real 360×800 keyboard/focus smoke `34246143800`; L00 baseline fresh checks pass, L01 next |
