...
test("GameplayEffect is a strict union of implemented executable effect types", async () => {
  ...
  assert.deepEqual(GAMEPLAY_EFFECT_TYPES, ["entity.move", "item.transfer", "resource.change"]);
  ...
  assert.equal(validate(move), true);
  assert.equal(isGameplayEffect(move), true);
  ...
  assert.equal(isEffect(move), false, "entity movement remains a typed GameplayEffect, not a generic Effect mutation");
});

test("entity.move rejects arbitrary mutation fields and malformed ids", () => {
  assert.equal(isGameplayEffect({
    schemaVersion: "1.0",
    type: "entity.move",
    sourceId: "task.return",
    entityId: "painter",
    locationId: "workshop",
    statePatch: { status: "anything" }
  }), false);
  assert.equal(isGameplayEffect({
    schemaVersion: "1.0",
    type: "entity.move",
