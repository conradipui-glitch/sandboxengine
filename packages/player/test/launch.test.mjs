...
async function seedSqliteStoreWithPlaytest(blocks) {
  ...
  assert.equal((await store.createQuest({
    projectId: "project", questId: "quest", title: "Тест", entryLocationId: "workshop", initialBlocks: [workshop]
  })).kind, "created");
  const applied = await store.applyDraftChanges("project", "quest", {
    baseRevision: 0,
  ...
  const created = await store.createPlaytest({
    projectId: "project", questId: "quest", draftRevision: 1, validationId: validated.validation.validationId
  });
...
test("L05 launch: start, repeat returns same address, close releases, relaunch works", async () => {
  ...
  try {
    ...
    const second = await launchFrozenPlayer({ databasePath, playtestId });
