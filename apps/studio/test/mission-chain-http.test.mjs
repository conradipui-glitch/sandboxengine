...
test("AI-CHAIN HTTP: подтверждённая цепочка доходит до писателя — предел идеи общий", async () => {
  ...
  try {
    ...
    // Писатель делит своё окно между двумя попытками, поэтому здесь видно
    // бюджет первой попытки: 540 с давали 270 с, и живой прогон на стенде
    // выгорел в timeout без документа. Окно 900 с даёт 450 с на попытку.
    // Допуск в 1 с: бюджет считается от остатка времени, и замер на прогоне
    // дал 449 999 мс — миллисекунда уходит между постановкой дедлайна и стартом.
    assert.ok(writerWindow >= 449_000, `окно попытки писателя мало: ${writerWindow} мс`);
  } finally {
    await close();
  }
});
