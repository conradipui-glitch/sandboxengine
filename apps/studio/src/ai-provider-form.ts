...
export const PROVIDER_CAUSE_TEXTS: Readonly<Record<ProviderCause, string>> = Object.freeze({
  ...
  generation_timeout:
    "Ключ и адрес верны, но модель не успела сгенерировать ответ за отведённое время: для больших ответов она слишком медленная. Повторите проверку или выберите в списке моделей более быструю.",
  generation_empty:
    "Ключ и адрес верны, но модель вернула пустой ответ: весь лимит вывода ушёл на размышления модели. Выберите модель без режима размышлений (в списке отмечены быстрые) и проверьте снова.",
  generation_truncated:
    "Ключ и адрес верны, но модель не договорила: ответ оборвался по лимиту вывода (часть бюджета ушла на размышления). Выберите более быструю модель без режима размышлений и проверьте снова.",
  generation_failed:
    "Ключ и адрес верны, но пробная генерация не удалась. Повторите проверку или выберите другую модель."
});
