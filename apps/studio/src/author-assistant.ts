...
export function renderAuthorAssistantPanel(
  ...
  if (state.kind === "empty") {
    const blockedReason = options.busy === true
      ? "Дождитесь завершения текущего шага."
      : options.canMutate && !options.hasMutationProof
        ? "Подтверждение входа устарело: обновите страницу и повторите."
        : "Для диалога нужна роль редактора или владельца.";
    const start = options.canMutate && options.hasMutationProof && options.busy !== true
      ...
      : `<div class="assistant-empty">${blockedReason}</div>`;
...
function jobFailedHint(code: string): string {
  ...
  if (code === "backend.auth_required") return " — ИИ не подключён или ключ отклонён: укажите провайдера, модель и ключ в блоке «Подключение ИИ-помощника».";
