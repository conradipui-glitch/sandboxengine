...
const SCREENS = [
  ...
  {
    id: "tour",
    title: "Тур по Studio (кнопка «Тур по Studio» в редакторе)",
    base: "editor",
    marker: `document.querySelector(".lh-tour-card") !== null`,
    route: [{ click: CLICK('[data-action="start-tour"]'), wait: 1300 }, { click: CLICK('[data-action="start-tour"]'), wait: 1200 }],
...
async function walkRoute(s) {
  ...
  await sleep(700);
  if (s.base === "projects") {
    for (const step of s.route) { if (step.click) await clickStep(step.click); await sleep(step.wait ?? 900); }
    return;
  }
...
async function ensureScreen(s) {
  ...
  if (s.base === "editor" && (await editorOpen())) {
    ...
    for (const step of s.route) { if (step.click) await evalJs(step.click); await sleep(step.wait ?? 900); }
    if (await matches(s)) return true;
  }
  await walkRoute(s);
  if (await matches(s)) return true;
