...
const appPath = "apps/studio/src/app.ts";
let app = await readFile(appPath, "utf8");
if (app.includes("renderAuthorAssistantPanel(this.state.authorAssistant")) throw new Error("Studio author app integration already applied");
...
`    versions: null,
    ...
    access: initialAccessState(),
`,
"initial author assistant state");
...
`    this.state.versions = null;
    ...
    try {
      this.state.quests = await this.api.listQuests(projectId);
`,
"select project assistant reset");
...
`        const fresh = await this.api.getDraft(projectId, questId);
        ...
        this.state.message = \`Draft изменился на сервере: \${draft.draftRevision} → \${fresh.draftRevision}. Ничего не перезаписано.\`;
`,
"saveChanges conflict assistant refresh");
...
`  private async refreshAuthorAssistant(projectId: string, questId: string): Promise<void> {
  ...
  private async refreshVersions(projectId: string, questId: string): Promise<void> {
`,
"author assistant controller methods");
...
function saveStateLabel(phase: StudioState["phase"]): string {
`,
"assistant busy helper");

await writeFile(appPath, app);

const restorePatchPath = appPath;
let restorePatched = await readFile(restorePatchPath, "utf8");
restorePatched = replaceOnce(restorePatched,
`      this.state.quests = await this.api.listQuests(projectId);
      await this.refreshVersions(projectId, questId);
      this.state.phase = "saved";
...
`      this.state.quests = await this.api.listQuests(projectId);
      ...
      this.state.message = \`r\${intent.sourceRevision} восстановлена как новая r\${restored.draftRevision}. Immutable releases не менялись.\`;
`,
"restore success assistant refresh");
restorePatched = replaceOnce(restorePatched,
`        this.state.publishReport = null;
        await this.refreshVersions(projectId, questId);
        this.state.phase = "conflict";
...
`        this.state.publishReport = null;
        ...
        this.state.message = \`Restore не выполнен: сервер уже на r\${fresh.draftRevision}. Ничего не перезаписано.\`;
`,
"restore conflict assistant refresh");
await writeFile(restorePatchPath, restorePatched);

const assistantPath = "apps/studio/src/author-assistant.ts";
let assistant = await readFile(assistantPath, "utf8");
assistant = replaceOnce(assistant,
...
`,
"assistant render options");
assistant = assistant.replaceAll(
`options: { readonly canMutate: boolean; readonly hasMutationProof: boolean }`,
`options: AuthorAssistantRenderOptions`
);
assistant = replaceOnce(assistant,
...
`    ${canSend ? \`<form class="assistant-composer" data-form="author-message">
      ...
      : \`<div class="assistant-empty">Assistant сейчас занят или mutation proof недоступен.</div>\`}
`,
"terminal new author job control");
assistant = replaceOnce(assistant,
...
`,
"proposal apply busy guard");
await writeFile(assistantPath, assistant);

const mainPath = "apps/studio/src/main.ts";
const currentMain = await readFile(mainPath, "utf8");
if (!currentMain.includes("SQLiteControlReleaseStore") || currentMain.includes("studio-dev-scripted-author")) throw new Error("unexpected Studio main shape");
const main = `// @ts-ignore — repository is pinned to Node 24.19.0; no @types/node dependency is installed yet.
...
const tsconfigPath = "apps/studio/tsconfig.json";
const tsconfig = JSON.parse(await readFile(tsconfigPath, "utf8"));
tsconfig.compilerOptions.paths["@living-history/ai"] = ["../../packages/ai/src/index.ts"];
if (!tsconfig.references.some((entry) => entry.path === "../../packages/ai")) {
  const serverIndex = tsconfig.references.findIndex((entry) => entry.path === "../server");
  tsconfig.references.splice(serverIndex < 0 ? tsconfig.references.length : serverIndex, 0, { path: "../../packages/ai" });
}
await writeFile(tsconfigPath, JSON.stringify(tsconfig, null, 2) + "\n");

const stylesPath = "apps/studio/styles.css";
let styles = await readFile(stylesPath, "utf8");
if (!styles.includes("/* B10.a Author Assistant */")) {
  styles += `
...
`;
await writeFile("apps/studio/test/author-assistant-app.test.mjs", integrationTest);

console.log("B10 Studio author app integration applied");
